...
Route::get('/hiviolations/oldsp', [TestController::class, 'migrasi']);
// Route::get('/hiviolations/olsp', [TestController::class, 'migrasi']);
...
Route::controller(UserController::class)->group(function () {
    ...
    Route::get('users/create', 'create')
        ->name('users.create')
        ->middleware('isrole');
    Route::get('users/{users:id}/edit', 'edit')
        ->name('users.edit')
        ->middleware('isrole');

    Route::get('users/{users:id}/password', 'password')
        ->name('users.password')
        ->middleware('isrole');
...
Route::get('users/yajra', [UserController::class, 'yajra'])->name(
    'users.yajra'
);
...
Route::resource('/employees', DatamasterEmployeeController::class)->middleware(
    'isrole'
);
...
Route::resource(
    '/promotions',
    DatamasterPromotionController::class
)->middleware('isrole');

Route::get('/promotions/{promotions:id}/getedit', [
    DatamasterPromotionController::class,
    'getedit',
])->middleware('isrole');
...
Route::controller(ExcelPromotionController::class)->group(function () {
    // Route::get('users', 'index');
    Route::get('exportpromotions', 'export')
        ->name('promotions.export')
        ->middleware('isrole');
    Route::post('importpromotions', 'import')
        ->name('promotions.import')
        ->middleware('isrole');
    Route::post('updatepromotions', 'update')
...
Route::get('/demotions/{demotions:id}/getedit', [
    DatamasterDemotionController::class,
    'getedit',
])->middleware('isrole');

//EXCEL IMPORT END EXPORT EMPLOYEES
Route::controller(ExcelDemotionController::class)->group(function () {
    Route::get('exportdemotions', 'export')
        ->name('demotions.export')
        ->middleware('isrole');
    Route::post('importdemotions', 'import')
        ->name('demotions.import')
        ->middleware('isrole');
    Route::post('updatedemotions', 'update')
        ->name('demotions.update')
        ->middleware('isrole');
});

// DATA MASTER MUTATION
Route::resource('/mutations', DatamasterMutationController::class)->middleware(
    'isrole'
);

Route::get('/mutations/{demotions:id}/getedit', [
    DatamasterMutationController::class,
    'getedit',
])->middleware('isrole');

//EXCEL IMPORT END EXPORT EMPLOYEES
Route::controller(ExcelMutationController::class)->group(function () {
    Route::get('exportmutations', 'export')
        ->name('mutations.export')
        ->middleware('isrole');
    Route::post('importmutations', 'import')
        ->name('mutations.import')
        ->middleware('isrole');
    Route::post('updatemutations', 'update')
        ->name('mutations.update')
        ->middleware('isrole');
});

// DATA EMPLOYE HI Violations
Route::resource('/violations', ViolationController::class)->middleware(
    'isrole'
);

// EDIT
Route::post('/violationeditexcel', [
    ViolationController::class,
    'violationeditexcel',
]);

// test SP
Route::get('/violations/{id}/testedit', [
    ViolationController::class,
    'testedit',
]);

// DATA VIOLATIONS
Route::get('/hiviolations', [
    HiViolationController::class,
    'index',
])->middleware('isrole');

Route::get('/hiviolations/yajra', [
    HiViolationController::class,
    'yajra',
])->name('hiviolations.yajra');

Route::resource('/hiviolations', HiViolationController::class)->middleware(
    'isrole'
);
...
Route::post('violation/get_type_violation', [
    ViolationController::class,
    'get_type_violation',
])->name('get_type_violation');

// ajax alphabet_id get on edit
Route::post('hiviolation/get_type_hiviolation', [
    HiViolationController::class,
    'get_type_hiviolation',
])->name('get_type_hiviolation');

Route::get('/teatviolations', [TeatViolationController::class, 'index']);

// ajax alphabet_id get on TeatViolationController
Route::post('/teatviolation/get_type_violation', [
    TeatViolationController::class,
    'get_type_violation',
])->name('get_type_teatviolation');

// Route::post('/teatviolation/get_type_violation', [TeatViolationController::class, 'get_type_violation'])->name('get_type_teatviolation');

Route::post('/teatviolation/store', [
    TeatViolationController::class,
    'store',
])->name('teatstore');
Route::post('/teatviolation/import', [
    TeatViolationController::class,
    'import',
])->name('teatimport');
Route::post('/teatviolation/update', [
    TeatViolationController::class,
    'update',
])->name('teatupdate');
Route::post('/teatviolation/updateimport', [
    TeatViolationController::class,
    'updateimport',
])->name('teatupdateimport');

///Migrasi Violation
Route::controller(ViolationmigrationController::class)->group(function () {
    Route::get('violationmigrations', 'index')->name(
        'violationmigrations.index'
    );
...
// PKB -> paragraphs
Route::resource('/paragraphs', HiParagraphController::class)->middleware(
    'isrole'
);

// PKB -> alphabets
Route::resource('/alphabets', HiAlphabetController::class)->middleware(
    'isrole'
);
...
Route::controller(HiAlphabetController::class)->group(function () {
    ...
    Route::post('importalphabets', 'import')
        ->name('alphabets.import')
        ->middleware('isrole');
    // Route::post('updatealphabets', 'update')->name('alphabets.update');
});

// SIGNATURES
Route::resource('/signatures', SignatureController::class)->middleware(
    'isrole'
);
Route::post('signatures/get_type_violation', [
    SignatureController::class,
    'get_signature',
])->name('get_signature');
Route::get('get_signature_employee', [
    SignatureController::class,
    'get_signature_employee',
])->name('get_signature_employee');

// LAYOFFS
Route::resource('/layoffs', LayoffController::class)->middleware('isrole');

Route::post('layoffs/get_karyawan_phk', [
    LayoffController::class,
    'get_karyawan_phk',
])
    ->name('get_karyawan_phk')
    ->middleware('isrole');

Route::get('layoffs/karyawan_phk', [LayoffController::class, 'karyawan_phk'])
    ->name('karyawan_phk')
    ->middleware('isrole');

Route::post('layoffs/get_pasal_phk', [LayoffController::class, 'get_pasal_phk'])
    ->name('get_pasal_phk')
    ->middleware('isrole');
...
// ACCESS MENUS
Route::resource('/access_menus', AccessMenuController::class)->middleware(
    'isrole'
);
Route::post('access_menus/changeaccess', [
    AccessMenuController::class,
    'changeaccess',
])
    ->name('changeaccess')
    ->middleware('isrole');
Route::post('access_menus/changeaccess_method', [
    AccessMenuController::class,
    'changeaccess_method',
])->name('changeaccess_method');
